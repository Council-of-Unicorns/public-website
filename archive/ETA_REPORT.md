...
**How to read the dated corrections.** This document keeps its mistakes in place, marked
and dated, rather than silently rewriting them — a correction you can audit is worth more
than a clean page. Text outside a marked correction block is current.
...
input's stated defensible interval, generated by `eta_range()` and rendered on the
explorer. Interval arithmetic, not a forecast: the breadth is the honest width of what
remains unmeasured, the floor is an all-adverse corner that misses the bar, and the four
design points are where judgment sits inside that box.

**Corrected 2026-08-10 (dense-Thor semantics; the change is favourable and cleared the
higher evidentiary bar we set for it).** NVIDIA's own product table states Thor's 2070
TFLOPS FP4 is the **sparse** rating; dense FP4 is 1035 = **7.96 TFLOP/W** [X, primary].
Our workload contract forbids structured sparsity, so the dense figure is the only peak
Thor can use against us — and the previous design points assumed Thor achieves 9.0
TFLOP/W, which *exceeds* its dense peak: impossible on this contract. Thor-achieved is now
anchored at 80% of the dense peak, the fraction our own measured RTX achieved of its
marketed dense peak on this workload (81%, rounded against us). Previous values (launch
2.0×, mature 2.9×) are superseded; the explorer now flags any Thor setting above the dense
peak as IMPOSSIBLE rather than pessimistic. The same
four numbers appear in the interactive explorer (where they are generated by
`scripts/explorer_data.py` and golden-checked), in `VC_CHEATSHEET.md`, and here; the
explorer is the authoritative rendering, and a mismatch between it and this prose is a
defect in the prose.
...
mixes accounting boundaries and is downgraded to a diagnostic; (c) **RESOLVED 2026-08-10**: Thor's sparse-vs-dense rating was confirmed against
NVIDIA's primary product table and the design points recomputed under dense semantics (see
the correction block above).
...
**When both modules are scored at the same power ceiling, the TDP ratio cancels and speedup
...
**The RPU module probably does not clear its own 2.15 bar on the day it powers on.** It clears it
...
**The ladder above the ceiling (named 2026-08-17).** Ladder vocabulary, binding across
all documents: *positions in the current design space* — floor 1.9×, bar 2.0×, target
...
*shrinks* the recomputed result, so the product bounds it from above.

| Rung | Construction | Envelope [T, upper bound] |
|---|---|---|
