...
|---|---|---|---|
| First silicon | 2.9× | 1.9–9.1× [S] | zero-instruction static schedule; FP4 systolic + FP8 attention datapath; weight-stream conveyor with CFG-pair sharing; fused attention; v1 compiler (0.55 extraction) |
| Mature compiler | 4.2× | 2.8–15.7×; 22× with gated sub-Vmin [S] | same silicon; compiler extraction 0.55 → 0.80 via scheduling, tail fusion, DMA overlap, idle gating |
| Frontier: 3D DRAM | — | ≤ 20–44× (bound, in escrow) [T] | model resident in hybrid-bonded stacked DRAM (reads ~40 → ~8 pJ/B); 8–16 GB over 100–200 MB distributed SRAM; bank-local compute deleting the central PHY/controller; moves the memory roofline |
