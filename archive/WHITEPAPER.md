...
of 1.9–15.7× (22× including gated physical-design upside) [S over T inputs] — the top
of that range is the program's north star, pursued but never underwritten. We present the RPU design point (4 PF dense FP4, LPDDR5X conveyor,
...
| Optimized | ~8× [T] | 6.0–9.7× [T] | model resident in hybrid-bonded stacked DRAM (reads ~40 → ~8 pJ/B); 8–16 GB over 100–200 MB distributed SRAM; bank-local compute deleting the central PHY/controller; moves the memory roofline |
| Codesign | ~12× [T] | 7–19× [T] | the model designed with the silicon: memory shaped to its reuse, operators as pipelines, precision only where needed |
| Ceiling | — | ~35–47× [S] | physics only: every joule in FP4 arithmetic at its floor — the absolute bound at matched precision; beyond it, gains must come from the model |
